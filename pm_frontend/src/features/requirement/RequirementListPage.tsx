  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        const data = (res.content as { data_list?: UserProfile[] }).data_list ?? []
        setRespSearchResults(data)
          ...
          <div>
            <div className="text-sm font-medium text-slate-700 mb-2">
              ...
              {respPersons.length > 0 && (
                <span className="ml-1.5 text-xs font-normal text-slate-400">{t('system.personsCount', { count: respPersons.length })}</span>
              )}
            </div>
            {respPreloading ? (
